...
demosaicked_rgb = demosaic_NEF(imgpath, imgshape = [3000, 4000], \
    white_level = 1023, black_level = 64, awb = [2.387, 1 , 1.498], gamma = 3)
'''
# show
...
imglist = get_files_NEFs('E:\\Deblur\\Short-Long RGB to RGB Mapping\\data\\slrgb2rgb_long10_short1_mobile_phone')
neflist = get_NEFs('E:\\Deblur\\Short-Long RGB to RGB Mapping\\data\\slrgb2rgb_long10_short1_mobile_phone')

for i in range(len(imglist)):
    print(i, imglist[i])
    demosaicked_rgb = demosaic_NEF(imglist[i], imgshape = [3000, 4000], \
        white_level = 1023, black_level = 64, awb = [2.387, 1 , 1.498], gamma = 3)
    last_len = len(imglist[i].split('\\')[-1])
    savepath = os.path.join(imglist[i][:-last_len], \
        neflist[i][:-3] + 'png')
    print(savepath)
    demosaicked_rgb = cv2.cvtColor(demosaicked_rgb, cv2.COLOR_RGB2BGR)
